...
- **Lighting changes after loading a saved game.** Saving and then loading a game results in noticeably darker lighting, making the scene difficult to see. The root cause is not yet identified. The DLL has no lighting code; all lighting state is managed by the game executable and its D3D render states. The D3D device stub returns `D3D_OK` for all `SetRenderState` calls without actually applying them, which may cause lighting state to be lost on load.
