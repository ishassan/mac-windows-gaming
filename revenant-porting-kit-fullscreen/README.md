...
- **Exit game now works.** Pressing Escape and clicking "Exit Game" exits cleanly. The root cause was that `present_frame()` was gated on `g_is_fullscreen`, so after `restore_window()` set that flag to FALSE during the exit sequence, the game could no longer render (exit confirmation dialogs, shutdown animations, etc.), causing it to appear frozen. The fix removes the `g_is_fullscreen` guard from `present_frame` (replaced with a window-dimensions check), and adds `SWP_ASYNCWINDOWPOS` to `SetWindowPos` in `restore_window()` to prevent cross-thread blocking with Wine's Mac Driver. `DD4_RestoreDisplayMode` and `DD4_SetCooperativeLevel(DDSCL_NORMAL)` now also post `WM_DISPLAYCHANGE` to the game window, and `SetCooperativeLevel` no longer overwrites `g_hwnd` with NULL.
...
- **Save game menu does not open.** During gameplay, pressing Escape and clicking "Save Game" does not open the save dialog. The root cause is not yet identified. The DLL has no dialog/menu code; all save functionality is in the game executable. Fallback surface creation and `FlipToGDISurface` have been added as defensive measures but did not resolve the issue.
